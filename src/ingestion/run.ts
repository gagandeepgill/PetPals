 ...
 * scheduling arrives with M1). Flow per listing:
 *   upsert organization -> upsert source_listing (content-hash diffed)
 *   -> normalize -> upsert pet via exact-key link -> photos.
 * Listings absent from a completed full sync are marked removed, and their
 * pets suppressed — de-listing is exactly as fresh as listing.
 */

const ADAPTERS: SourceAdapter[] = [rescueGroupsAdapter];
...
async function upsertPet(listing: RawListing, listingId: string, orgId: string): Promise<string> {
  ...
  const values = [
    ...
    n.traits, n.description, n.status, orgId, "RescueGroups", n.sourceUrl,
...
async function revalidate(petIds: string[]): Promise<void> {
  ...
  }).catch((err) => ctx.log(`revalidate failed: ${err}`));
}
...
async function main() {
  ...
  for (const adapter of ADAPTERS) {
    ...
    if (!health.ok) {
      ctx.log(`${adapter.sourceId} healthcheck failed: ${health.detail ?? "unknown"}`);
      continue;
    }
    ...
    const removed = await suppressUnseen(adapter.sourceId, runStart);
    await revalidate(changedPets);
    ctx.log(
      `${adapter.sourceId}: fetched=${fetched} changed=${changed} removed=${removed} in ${Math.round((Date.now() - runStart.getTime()) / 1000)}s`,
    );
  }
  await getPool().end();
}
